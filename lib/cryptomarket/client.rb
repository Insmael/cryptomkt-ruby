...
module Cryptomarket
  class Client
    ...
    #
    # ==== Params
    # +String+ +client order id+:: The client order id of the order
    ...
    # +String+ +client order id+:: Optional. If given must be unique within the trading day, including all active orders. If not given, is generated by the server
    # +String+ +type+:: Optional. 'limit', 'market', 'stopLimit', 'stopMarket', 'takeProfitLimit' or 'takeProfitMarket'. Default is 'limit'
    # +String+ +time in force+:: Optional. 'GTC', 'IOC', 'FOK', 'Day', 'GTD'. Default to 'GTC'
    # +String+ +price+:: Optional. Required for 'limit' and 'stopLimit'. limit price of the order
    # +String+ +stop price+:: Optional. Required for 'stopLimit' and 'stopMarket' orders. stop price of the order
    # +String+ +expire time+:: Optional. Required for orders with timeInForce = GDT
    # +bool+ +strict validate+:: Optional. If False, the server rounds half down for tickerSize and quantityIncrement. Example of ETHBTC: tickSize = '0.000001', then price '0.046016' is valid, '0.0460165' is invalid
    # +bool+ +post only+:: Optional. If True, your post_only order causes a match with a pre-existing order as a taker, then the order will be cancelled
    # +String+ +take rate+:: Optional. Liquidity taker fee, a fraction of order volume, such as 0.001 (for 0.1% fee). Can only increase the fee. Used for fee markup.
    # +String+ +make rate+:: Optional. Liquidity provider fee, a fraction of order volume, such as 0.001 (for 0.1% fee). Can only increase the fee. Used for fee markup.
    ...
    #
    # ==== Params
    # +String+ +client order id+:: client order id of the old order
    # +String+ +new client order id+:: client order id for the new order
    # +String+ +quantity+:: Order quantity
    # +bool+ +strict validate+:: Price and quantity will be checked for incrementation within the symbol’s tick size and quantity step. See the symbol's tick_size and quantity_increment
    ...
    def cancel_all_spot_orders(symbol:nil)
      return delete("spot/order", {symbol:symbol})
    end
    ...
    #
    # ==== Params
    # +String+ +client order id+:: client order id of the order to cancel
    ...
    #
    # ==== Params
    # +String+ +currency+:: Optional. Code of the dropped currency
    # +String+ +base currency+:: Optional. The code of base currency (the currency used for dropped currency amount calculation)
    # +String+ +active at+:: Optional. Default is value is active at current time
    # +Array[String]+ +statuses+:: Optional. A list of desired airdrop statuses. Accepted values are: 'available' (ready for claim), 'claimed' (requested already), 'pending' (payment in progress) and 'commited' (payment finished)
    # +String+ +transaction id+:: Optional. Airdrop transaction identifier
    def get_airdrops(
      currency:nil,
      base_currency:nil,
      activa_at:nil,
    ...
    #
    # ==== Params
    # +String+ +currency+:: Code of the dropped currency

    def cliam_airdrop(id:, currency:)
      return post("wallet/airdrops/#{id}/claim", {currency:currency})
    end
